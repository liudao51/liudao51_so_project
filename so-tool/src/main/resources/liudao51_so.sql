...
## 文章快照表(article_snapshot)
CREATE TABLE `article_snapshot`
(
    `id`           BIGINT(20)    NOT NULL COMMENT '文章快照id',
    `article_id`   BIGINT(20)    NOT NULL COMMENT '文章id',
    `content`      VARCHAR(5000) NOT NULL DEFAULT '' COMMENT '内容',
    `created_time` BIGINT(20)    NOT NULL DEFAULT 0 COMMENT '创建时间',
    `updated_time` BIGINT(20)    NOT NULL DEFAULT 0 COMMENT '更新时间',
  ...
  DEFAULT CHARSET = utf8 COMMENT ='文章快照';


############################################
#article
INSERT INTO `article` (`id`, `title`, `description`, `site_url`) VALUES (1, 'GitHub 上有哪些优秀的 Java 爬虫项目? - 知乎', 'WebCollector是一个无须配置、便于二次开发的JAVA爬虫框架(内核),它提供精简的的API,只需少量代码即可实现一个功能强大的爬虫。WebCollector-Hadoop是WebCollector的','https://www.zhihu.com/question/31427895');
INSERT INTO `article` (`id`, `title`, `description`, `site_url`) VALUES (1, 'Java爬虫_阳神-CSDN博客_java爬虫', 'Java爬虫 网络爬虫 概念 网络爬虫(Web crawler),是一种按照一定的规则,自动地抓取万维网信息的程序或者脚本。 简单的小程序 创建依赖...','https://blog.csdn.net/qq_44797965/article/details/108251164');
INSERT INTO `article` (`id`, `title`, `description`, `site_url`) VALUES (2, 'Python爬虫原理', '简单来说互联网是由一个个站点和网络设备组成的大网，我们通过浏览器访问站点，站点把HTML、JS、CSS代码返回给浏览器，这些代码经过浏览器解析、渲染，将丰富多彩的网页呈现我们眼前','https://www.cnblogs.com/sss4/p/7809821.html');
INSERT INTO `article` (`id`, `title`, `description`, `site_url`) VALUES (2, '入门Python爬虫——获取数据篇', '在如今的大数据时代，相信大家都对Python一词有所耳闻。而Python爬虫，简单来说，即通过Python程序获取对我们有用的数据。常用于商业分析，不过偶尔也可以辅助我们解决在日常生活中遇到的一些问题。','https://baijiahao.baidu.com/s?id=1670801762859023601');
#article_keyword
...
#article_snapshot
INSERT INTO `article_snapshot` (`id`, `article_id`, `content`) VALUES (1, '1', '<blockquote>Gecco是一款用java语言开发的轻量化的易用的网络爬虫。整合了jsoup、httpclient、fastjson、spring、htmlunit、redission等框架，只需要配置一些jquery风格的选择器就能很快的写出一个爬虫。Gecco框架有优秀的可扩展性，框架基于开闭原则进行设计，对修改关闭、对扩展开放。<br></blockquote>');
INSERT INTO `article_snapshot` (`id`, `article_id`, `content`) VALUES (2, '1', '<p>在大数据时代，信息的采集是一项重要的工作，而互联网中的数据是海量的，如果单纯靠人力进行信息采集，不仅低效繁琐，搜集的成本也会提高。如何自动高效地获取互联网中我们感兴趣的信息并为我们所用是一个重要的问题，而爬虫技术就是为了解决这些问题而生的</p>');
INSERT INTO `article_snapshot` (`id`, `article_id`, `content`) VALUES (3, '2', '<p>简单来说互联网是由一个个站点和网络设备组成的大网，我们通过浏览器访问站点，站点把HTML、JS、CSS代码返回给浏览器，这些代码经过浏览器解析、渲染，将丰富多彩的网页呈现我们眼前；</p>');
INSERT INTO `article_snapshot` (`id`, `article_id`, `content`) VALUES (4, '2', '<p bdsfid="59"><span class="bjh-p" bdsfid="60">在如今的大数据时代，相信大家都对Python一词有所耳闻。而<span class="bjh-strong" bdsfid="61"><b style="color:black;background-color:#ffff66">Python爬虫</b></span>，简单来说，即<span class="bjh-strong" bdsfid="62">通过Python程序获取对我们有用的数据</span>。常用于商业分析，不过偶尔也可以辅助我们解决在日常生活中遇到的一些问题。</span></p>');
